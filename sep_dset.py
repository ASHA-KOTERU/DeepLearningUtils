...
def rec_files_from_path(path, num_db_files = None):
  ...
  if path.endswith('.txt'):
    ...
    rec_files = sorted(ut.glob(path, '*.mp4'))
    # rec_files = sorted(ut.glob(path, '*.tf'))
  return rec_files
  
def make_db_reader(path, pr, batch_size, input_types, db_start = None,
                   num_db_files = None, num_threads = 12, one_pass = True):
  print 'one pass =', one_pass
  print "in make_db_reader"
  print "path",path
  print "pr",pr
  print "batch_size",batch_size
  print "input_types",input_types
  print "db_start",db_start
  print "num_db_files",num_db_files
